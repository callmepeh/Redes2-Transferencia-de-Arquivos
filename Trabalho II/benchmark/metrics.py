...
conforme exigido pelo enunciado (10+ execuções por cenário).
"""
...
def summarize_results(results: list[dict]) -> dict:
    """
    Recebe uma lista de dicts (cada um com time, throughput, retransmissions, dns_time)
    e retorna um resumo estatístico.
    """
    times = [r["time"] for r in results]
    throughputs = [r["throughput"] for r in results]
    retransmissions = [r["retransmissions"] for r in results]
    dns_times = [r.get("dns_time", 0) for r in results]
    http_sizes = [r.get("body_size", 0) for r in results]
    ...
    return {
        ...
        "dns_time": compute_stats(dns_times),
        "http_size": compute_stats(http_sizes),
    }
